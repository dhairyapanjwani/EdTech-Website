    ...
    return (
            <div class="p-4 md:w-1/3">
              <div class="h-full border-2 border-gray-200 dark:border-gray-600 border-opacity-60 rounded-lg overflow-hidden">
                <img class="lg:h-48 md:h-36 w-full object-cover object-center" src={img} alt="blog" onClick={() => history.push(`courses/${_id}`)} style={{ cursor: 'pointer' }}/>
                <div class="p-6">
                  <h2 class="tracking-widest text-xs title-font font-medium text-gray-400 mb-1 uppercase" onClick={() => history.push(`courses/${_id}`)} style={{ cursor: 'pointer' }}>{category}</h2>
                  {/* <div className="flex flex-row justify-between align-center">
                    <div className="text-indigo-500 text-base mt-1 underline">Add to Cart</div>
                  </div> */}
                  <h1 class="title-font text-lg font-medium text-gray-900 dark:text-gray-200 mb-3" onClick={() => history.push(`courses/${_id}`)} style={{ cursor: 'pointer' }}>{title}</h1>
                  <p class="leading-relaxed mb-3 dark:text-gray-500 float-none" onClick={() => history.push(`courses/${_id}`)} style={{ cursor: 'pointer' }}>{description}</p>
                  <div class="flex items-center flex-wrap ">
                    ...
                    <span class="text-gray-400 mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm pr-3 py-1 border-r-2 border-gray-200" onClick={async () => {
                      const data = await CourseApi.likeIt('614adf467e4020a62c1b157d', _id);
                      console.log(data);
                    }}>
                      {present === true ? (<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
