...
const Form=({todos,setTodos,inputText,setInputText,setStatus})=>{
    ...
    return(
        <form className="m-4 flex">
    	    <input value={inputText} onChange={inputTextHandler}  type="text" className="rounded-l-lg shadow-inner text-gray-800 bg-gray-100" placeholder="Enter Task . . ."/>
		    <button typr="submit" onClick={submitTododHandler} className=" rounded-r-lg bg-indigo-500 hover:bg-indigo-600 px-3 text-gray-800 font-bold uppercase border-indigo-500 border-t border-b border-r"><i className="add-icon fa fa-plus-square"></i></button>
